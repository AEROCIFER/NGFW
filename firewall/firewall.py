...
from scapy.all import sniff, TCP, IP, Raw, DNSQR, DNS, get_if_addr, get_if_list, conf
import re
import netifaces 
...
connections = set()  # stores edges (src, dst)

def learn_topology(packet):
    if packet.haslayer(IP):
        src = packet[IP].src
        dst = packet[IP].dst
        # Add a tuple edge (src → dst)
        connections.add((src, dst))

# Learn for 30 seconds before starting firewall
print("Learning network topology...")
sniff(filter="ip", prn=learn_topology, store=0, timeout=30)
print("Learned connections:", connections)
...
pos = nx.spring_layout(G, seed=42)  # positions for a nice layout
